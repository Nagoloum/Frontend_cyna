...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!data?.success) {
        notify.error(t('forgotPassword.request_failed'), data?.message || t('forgotPassword.send_error'));
        return;
      }
            ...
            {isSubmitted ? (
              <div className="text-center py-0">
                ...
                <p className="text-gray-600 dark:text-gray-300 mb-4">
                  {t('forgotPassword.sent_to')} <span className="font-medium">{email}</span>
                </p>
