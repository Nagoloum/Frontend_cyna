...
function Thumb({ src, alt, size = 9, placeholder: Placeholder = Package }) {
  ...
  return (
    <div
      className={`w-${size} h-${size} rounded-xl flex-shrink-0 overflow-hidden bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center border border-gray-100 dark:border-gray-700/60`}
    >
      {url && !error ? (
        <img
          src={url}
        ...
        {[...Array(totalPages)].map((_, i) => {
          ...
          if (!near) {
            if (p === 2 || p === totalPages - 1)
              return (
                <span key={p} className="text-gray-400 px-1 text-xs">
                  …
                </span>
              );
            return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <h3 className="text-base font-bold text-gray-900 dark:text-white mb-1">
          {t("admin.common.delete_title")}
        </h3>
        <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 my-3 px-3 py-2 bg-gray-50 dark:bg-gray-700 rounded-xl truncate">
          {name}
        </p>
        <p className="text-xs text-red-500 mb-5">
          ⚠ {t("admin.common.delete_body")}
        </p>
        <div className="flex gap-3">
          ...
          <button
            onClick={run}
            disabled={loading}
            className="flex-1 h-10 rounded-xl text-sm font-medium text-white bg-red-500 hover:bg-red-600 disabled:opacity-50 transition-all flex items-center justify-center gap-2"
          >
...
function SliderFormModal({ slider, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    if (!form.title.trim())
      return setError(t("admin.sliders.err_title_required"));
    if (!form.nameUrl.trim())
      return setError(t("admin.sliders.err_button_required"));
    if (!form.linkUrl.trim())
      return setError(t("admin.sliders.err_link_required"));
    if (!isEdit && !imageFile)
      return setError(t("admin.sliders.err_image_required"));
    ...
    try {
      ...
      notify.success(
        isEdit ? t("admin.sliders.updated") : t("admin.sliders.created"),
        form.title.trim(),
      );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div>
            <h2 className="text-base font-bold text-gray-900 dark:text-white">
              {isEdit
                ? t("admin.sliders.edit_modal_title")
                : t("admin.sliders.new_modal_title")}
            </h2>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              {t("admin.sliders.modal_subtitle")}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {error && (
            <div className="flex items-start gap-2 p-3 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-200 text-red-700 dark:text-red-400 text-sm">
              ...
              <div className="flex-1">
                ...
                >
                  ...
                  {imageFile
                    ? imageFile.name
                    : isEdit
                      ? t("admin.common.change_image")
                      : t("admin.common.choose_image")}
                </button>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                  {t("admin.common.image_hint")}
                </p>
              </div>
            </div>
            <input
          ...
          <div>
            ...
            <input
              ...
              placeholder={t("admin.sliders.placeholder_title")}
              className={inp}
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              value={form.nameUrl}
              onChange={handleChange}
              placeholder={t("admin.sliders.placeholder_button")}
              className={inp}
            />
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
              {t("admin.sliders.label_button_hint")}
            </p>
          </div>
...
function SlidersTab() {
  ...
  const fetchSliders = useCallback(async () => {
    ...
    try {
      ...
      setError(e.response?.data?.message ?? t("admin.sliders.load_failed"));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  return (
    <div className="space-y-4">
      ...
      {error && (
        <div className="flex items-center gap-2 p-3 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-200 text-red-700 dark:text-red-400 text-sm">
          <AlertCircle size={14} />
          {error}
          <button
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {[0, 1, 2].map((i) => (
            <div
              key={i}
              className="rounded-2xl bg-gray-100 dark:bg-gray-700 animate-pulse h-48"
            />
          ))}
        </div>
      ) : sliders.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {sliders.map((s) => (
            <div
              key={s._id}
              className="group bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700/60 shadow-sm overflow-hidden flex flex-col"
            >
              {/* Preview */}
              <div className="relative h-36 bg-gray-100 dark:bg-gray-700 overflow-hidden flex-shrink-0">
                {s.image ? (
                  <img
                    src={buildImageUrl(s.image)}
                    alt={s.title}
...
function ProductsTab({ categories, services }) {
      ...
      try {
        const params = {
          ...
          ...((opts.filterCategory ?? filterCategory)
            ? { categorySlug: opts.filterCategory ?? filterCategory }
            : {}),
        };
        ...
        setPagination((p) => ({
          ...p,
          page: params.page,
          total: d?.data?.total ?? d?.total ?? items.length,
        }));
        setProducts(items);
      } catch (e) {
        setError(e.response?.data?.message ?? t("admin.products.load_failed"));
      } finally {
        setLoading(false);
      }
    },
  ...
  useEffect(() => {
    const onChange = (e) => {
      if (
        !e?.detail?.kind ||
        ["product", "category", "service"].includes(e.detail.kind)
      ) {
        fetch();
      }
    };
  ...
  const onSort = (f) => {
    ...
    setSortBy(f);
    setSortOrder(o);
    fetch({ sortBy: f, sortOrder: o, page: 1 });
  };
  const onSaved = () => {
    setModal(null);
    fetch({ page: 1 });
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="w-44">
          <AdminSelect
            ...
            onChange={(e) => {
              ...
              fetch({ filterCategory: e.target.value, page: 1 });
            }}
          >
        ...
        <span className="text-xs text-gray-400 dark:text-gray-500 ml-auto hidden sm:block">
          {t("admin.products.count", { count: pagination.total })}
        </span>
        <button
          onClick={() => setModal({ type: "create" })}
          className="flex items-center gap-1.5 h-9 px-4 rounded-xl text-sm font-medium text-white bg-indigo-500 hover:bg-indigo-600 transition-all"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700/60">
                ...
                />
                <Th
                  label={t("admin.products.col_service")}
                  field="service"
                  sortBy={sortBy}
                  order={sortOrder}
                  onSort={onSort}
                ...
                <Th
                  label={t("admin.products.col_monthly")}
                  field="priceMonth"
                  sortBy={sortBy}
                  order={sortOrder}
                  onSort={onSort}
                />
                <Th
                  label={t("admin.products.col_yearly")}
                  field="priceYear"
                  sortBy={sortBy}
                  order={sortOrder}
                  onSort={onSort}
                ...
                <Th
                  label={t("admin.common.created_at")}
                  field="createdAt"
                  sortBy={sortBy}
                  order={sortOrder}
                  onSort={onSort}
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700/40">
              {loading ? (
                ...
                <EmptyState
                  icon={Package}
                  message={t("admin.products.empty")}
                  sub={
                    search
                      ? t("admin.products.empty_sub_search")
                ...
                products.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-semibold text-gray-900 dark:text-white">
                      {fmtPrice(p.priceMonth)}
                    </td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">
                      {fmtPrice(p.priceYear)}
                    </td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">
                      {p.stock ?? "∞"}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                        <button
                          ...
                          className="p-1.5 rounded-lg text-gray-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 hover:text-indigo-600 transition-all"
                        >
                          <Edit2 size={13} />
                        </button>
                        <button
                          onClick={() => setModal({ type: "delete", data: p })}
                          className="p-1.5 rounded-lg text-gray-400 hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-500 transition-all"
                        >
                          <Trash2 size={13} />
                        </button>
                      </div>
...
function ServiceFormModal({ service, categories, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    if (!form.name.trim())
      return setError(t("admin.services.err_name_required"));
    if (!form.categoryId)
      return setError(t("admin.services.err_category_required"));
    setLoading(true);
    try {
      const payload = {
        name: form.name.trim(),
      ...
      notify.success(
        isEdit ? t("admin.services.updated") : t("admin.services.created"),
        form.name.trim(),
      );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-base font-bold text-gray-900 dark:text-white">
            {isEdit
              ? t("admin.services.edit_modal_title")
              : t("admin.services.new_modal_title")}
          </h2>
          <button
            onClick={onClose}
            className="p-2 rounded-xl text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              value={form.name}
              onChange={handleChange}
              placeholder={t("admin.services.placeholder_name")}
              className={inp}
            />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">
              {t("admin.services.label_category")}{" "}
              <span className="text-red-500">*</span>
            </label>
            <AdminSelect
              name="categoryId"
              ...
              onChange={handleChange}
            >
              <option value="">{t("admin.services.select_category")}</option>
              {categories.map((c) => (
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">
              {t("admin.services.label_description")}
            </label>
            <textarea
              name="description"
              ...
              className="w-full px-3 py-2.5 rounded-xl text-sm bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 resize-none"
            />
          </div>
          <label className="flex items-center gap-2.5 cursor-pointer">
            <div className="relative">
            ...
            <span className="text-sm text-gray-700 dark:text-gray-300">
              {t("admin.services.label_available")}
            </span>
          </label>
          <div className="flex gap-3 pt-2 border-t border-gray-200 dark:border-gray-700">
            <button
...
function ServicesTab({ categories }) {
  ...
  const fetch = useCallback(
    async (opts = {}) => {
      ...
      try {
        ...
        setError(e.response?.data?.message ?? t("admin.services.load_failed"));
      } finally {
        setLoading(false);
      }
    },
  ...
  useEffect(() => {
    const onChange = (e) => {
      if (!e?.detail?.kind || ["service", "category"].includes(e.detail.kind))
        fetch();
    };
    const onRefresh = () => fetch();
    window.addEventListener(ADMIN_DATA_EVENT, onChange);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700/60 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-700/60">
              <Th label={t("admin.services.col_service")} />
              <Th label={t("admin.services.col_category")} />
              <Th label={t("admin.services.col_description")} />
              <Th label={t("admin.services.col_availability")} />
              <Th label={t("admin.common.created_at")} />
              <Th label="" />
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700/40">
            {loading ? (
              ...
              <EmptyState
                icon={Layers}
                message={t("admin.services.empty")}
                sub={t("admin.services.empty_sub")}
              />
            ) : (
              filtered.map((s) => (
                <tr
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <StatusPill active={s.available !== false} />
                  </td>
...
function CategoryFormModal({ category, onClose, onSaved, nextOrder }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    if (!form.name.trim())
      return setError(t("admin.categories.err_name_required"));
    if (!isEdit && !imageFile)
      return setError(t("admin.categories.err_image_required"));
    setLoading(true);
    try {
      const fd = new FormData();
      ...
      notify.success(
        isEdit ? t("admin.categories.updated") : t("admin.categories.created"),
        form.name.trim(),
      );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-base font-bold text-gray-900 dark:text-white">
            {isEdit
              ? t("admin.categories.edit_modal_title")
              : t("admin.categories.new_modal_title")}
          </h2>
          <button
            onClick={onClose}
            className="p-2 rounded-xl text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">
              {t("admin.categories.label_image")}{" "}
              {!isEdit && <span className="text-red-500">*</span>}
            </label>
            <div className="flex items-center gap-3">
              <div
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">
                {t("admin.categories.label_order")}
              </label>
              <input
                name="order"
                type="number"
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">
              {t("admin.categories.label_description")}
            </label>
            <textarea
              name="description"
              value={form.description}
...
function CategoriesTab() {
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      ...
      setError(e.response?.data?.message ?? t("admin.categories.load_failed"));
    } finally {
      setLoading(false);
    }
  }, [t]);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700/60 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-700/60">
              <Th label={t("admin.categories.col_category")} />
              <Th label={t("admin.categories.col_slug")} />
              <Th label={t("admin.categories.col_description")} />
              <Th label={t("admin.categories.col_order")} />
              <Th label={t("admin.common.created_at")} />
              <Th label="" />
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700/40">
            {loading ? (
              ...
              <EmptyState
                ...
                sub={t("admin.categories.empty_sub")}
              />
            ) : (
              categories.map((cat) => (
                <tr
...
const TABS = [
  ...
  {
    id: "sliders",
    labelKey: "admin.catalog.tab_sliders",
...
export default function ProductsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5">
      ...
      {/* Tab content */}
      {tab === "products" && (
        <ProductsTab categories={categories} services={services} />
      )}
