...
import {
  ...
  Check, Image as ImageIcon, AlertTriangle,
  ToggleLeft, ToggleRight, Eye, EyeOff,
} from 'lucide-react';
import { useSearchParams } from 'react-router-dom';
import { productsAPI, categoriesAPI, servicesAPI, buildImageUrl, extractList } from '../../services/api';
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-6">
        <button onClick={onClose} className="absolute top-4 right-4 p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><X size={15} /></button>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">You are about to delete:</p>
        <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-4 px-3 py-2 bg-gray-50 dark:bg-gray-700 rounded-xl truncate">{name}</p>
        <p className="text-xs text-red-500 mb-5">⚠ This action cannot be undone.</p>
        <div className="flex gap-3">
          <button onClick={onClose} disabled={loading} className="flex-1 h-10 rounded-xl text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 disabled:opacity-50 transition-all">Cancel</button>
...
function ProductFormModal({ product, categories, services, onClose, onSaved }) {
  ...
  const handleSubmit = async e => {
    ...
    if (!form.priceMonth && !form.priceYear) return setError('At least one price (monthly or yearly) is required.');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-xl max-h-[90vh] overflow-y-auto bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700">
        <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
          ...
          <button onClick={onClose} className="p-2 rounded-xl text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><X size={16} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            {services.length === 0 && <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">⚠ No services found. Create a service first.</p>}
          </div>

          {/* Prices + Stock */}
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Monthly (€)</label>
              <input name="priceMonth" type="number" min="0" step="0.01" value={form.priceMonth} onChange={handleChange} placeholder="0.00" className={inp} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Yearly (€)</label>
              <input name="priceYear" type="number" min="0" step="0.01" value={form.priceYear} onChange={handleChange} placeholder="0.00" className={inp} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Stock</label>
              <input name="stock" type="number" min="0" value={form.stock} onChange={handleChange} placeholder="∞" className={inp} />
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            {[
              { name: 'is_selected', label: 'Active / Visible' },
              { name: 'priority',    label: 'Featured'         },
            ].map(({ name, label }) => (
          ...
          <div>
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {previews.map((url, i) => (
                  <div key={i} className="relative group w-20 h-20 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700">
                    ...
                    <button type="button" onClick={() => removeImage(i)} className="absolute inset-0 flex items-center justify-center bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity text-white">
                      <Trash2 size={15} />
                    </button>
                  </div>
                ))}
            ...
            <button type="button" onClick={() => fileRef.current?.click()}
              className="w-full flex items-center justify-center gap-2 h-10 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-600 text-sm text-gray-500 dark:text-gray-400 hover:border-indigo-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-500/5 transition-all">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Images stored at: <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded">public/uploads/products/</code></p>
          </div>
...
function ProductsTab({ categories, services }) {
  ...
  useEffect(() => {
    if (searchParams.get('action') === 'create') setModal({ type: 'create' });
  }, [searchParams]);

  const onSearch = v => {
    setSearch(v);
    clearTimeout(searchRef.current);
    searchRef.current = setTimeout(() => fetch({ search: v, page: 1 }), 400);
  };
  const onSort = f => {
    const o = sortBy === f && sortOrder === 'asc' ? 'desc' : 'asc';
    setSortBy(f); setSortOrder(o); fetch({ sortBy: f, sortOrder: o, page: 1 });
  };
  const onSaved = () => { setModal(null); fetch({ page: 1 }); };
  const onDelete = async () => {
    await productsAPI.delete(modal.data.slug);
    setModal(null); fetch({ page: pagination.page });
  };
  ...
  return (
    <div className="space-y-4">
              ...
              {loading ? [...Array(5)].map((_, i) => <SkeletonRow key={i} cols={8} />)
                ...
                : products.map(p => (
                  <tr key={p.slug ?? p._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-sm text-gray-600 dark:text-gray-300">{p.service?.name ?? '—'}</span>
                    </td>
      ...
      {(modal?.type === 'create' || modal?.type === 'edit') && (
        <ProductFormModal product={modal.data} categories={categories} services={services}
          onClose={() => setModal(null)} onSaved={onSaved} />
      )}
      {modal?.type === 'delete' && (
        <DeleteModal name={modal.data.name} onClose={() => setModal(null)} onConfirm={onDelete} />
      )}
    </div>
  );
}
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Services are linked to a category</p>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><X size={15} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Name <span className="text-red-500">*</span></label>
            <input name="name" value={form.name} onChange={handleChange} placeholder="e.g. EDR Protection" className={inp} />
          </div>
          <div>
            ...
            <select name="categoryId" value={form.categoryId} onChange={handleChange} className={inp}>
              <option value="">Select a category…</option>
              {categories.map(c => <option key={c._id ?? c.slug} value={c._id ?? c.slug}>{c.name}</option>)}
            </select>
            {categories.length === 0 && <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">⚠ No categories found. Create a category first.</p>}
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Description</label>
            <textarea name="description" value={form.description} onChange={handleChange} rows={3} placeholder="Optional description…"
              className="w-full px-3 py-2.5 rounded-xl text-sm bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 transition-all resize-none" />
          </div>
          <label className="flex items-center gap-2.5 cursor-pointer">
            <div className="relative">
              <input type="checkbox" name="available" checked={form.available} onChange={handleChange} className="sr-only peer" />
              <div className="w-9 h-5 rounded-full bg-gray-200 dark:bg-gray-600 peer-checked:bg-indigo-500 after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:w-4 after:h-4 after:transition-all peer-checked:after:translate-x-4 transition-colors" />
            </div>
...
function ServicesTab({ categories }) {
  ...
  const fetch = useCallback(async (opts = {}) => {
    setLoading(true); setError(null);
    try {
      const res = await servicesAPI.getAll(opts.params ?? {});
      setServices(extractList(res.data));
  ...
  return (
    <div className="space-y-4">
      ...
      {error && <div className="flex items-center gap-2 p-3 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-200 text-red-700 dark:text-red-400 text-sm"><AlertCircle size={14} />{error}<button onClick={() => fetch()} className="ml-auto text-xs underline">Retry</button></div>}
              ...
              {loading ? [...Array(4)].map((_, i) => <SkeletonRow key={i} cols={6} />)
                ...
                : filtered.map(s => (
                  <tr key={s.slug ?? s._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-violet-50 dark:bg-violet-500/10 flex items-center justify-center flex-shrink-0 border border-violet-100 dark:border-violet-500/20">
                          <Layers size={14} className="text-violet-500" />
                        </div>
                        <div className="min-w-0">
                          <p className="font-medium text-gray-900 dark:text-white truncate max-w-[160px]">{s.name}</p>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                        <button onClick={() => setModal({ type: 'edit', data: s })} className="p-1.5 rounded-lg text-gray-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 hover:text-indigo-600 transition-all"><Edit2 size={13} /></button>
                        <button onClick={() => setModal({ type: 'delete', data: s })} className="p-1.5 rounded-lg text-gray-400 hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-500 transition-all"><Trash2 size={13} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {(modal?.type === 'create' || modal?.type === 'edit') && (
        <ServiceFormModal service={modal.data} categories={categories} onClose={() => setModal(null)} onSaved={onSaved} />
      )}
      {modal?.type === 'delete' && (
        <DeleteModal name={modal.data.name} onClose={() => setModal(null)} onConfirm={onDelete} />
      )}
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Images saved to <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded">public/uploads/category/</code></p>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><X size={15} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div
                ...
                className="w-20 h-20 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-600 flex items-center justify-center overflow-hidden cursor-pointer hover:border-indigo-400 transition-colors flex-shrink-0 bg-gray-50 dark:bg-gray-700/50"
              >
                {preview
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2">
              <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Name <span className="text-red-500">*</span></label>
              <input name="name" value={form.name} onChange={handleChange} placeholder="e.g. Endpoint Security" className={inp} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Order</label>
              <input name="order" type="number" min="1" value={form.order} onChange={handleChange} className={inp} />
            </div>
          </div>
...
function CategoriesTab() {
  ...
  return (
    <div className="space-y-4">
                ...
                : categories.map(cat => (
                  <tr key={cat.slug ?? cat._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        {/* Image thumbnail */}
                        <div className="w-10 h-10 rounded-xl flex-shrink-0 overflow-hidden border border-gray-100 dark:border-gray-700/60 bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center">
                          {cat.image
                            ? <img src={buildImageUrl(cat.image)} alt={cat.name} className="w-full h-full object-cover" onError={e => { e.target.style.display='none'; }} />
                    ...
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-xs font-bold">
                        {cat.order ?? '—'}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                        <button onClick={() => setModal({ type: 'edit', data: cat })} className="p-1.5 rounded-lg text-gray-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 hover:text-indigo-600 transition-all"><Edit2 size={13} /></button>
                        <button onClick={() => setModal({ type: 'delete', data: cat })} className="p-1.5 rounded-lg text-gray-400 hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-500 transition-all"><Trash2 size={13} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {(modal?.type === 'create' || modal?.type === 'edit') && (
        <CategoryFormModal category={modal.data} nextOrder={nextOrder} onClose={() => setModal(null)} onSaved={onSaved} />
      )}
      {modal?.type === 'delete' && (
        <DeleteModal name={modal.data.name} onClose={() => setModal(null)} onConfirm={onDelete} />
      )}
...
export default function ProductsPage() {
  ...
  // Shared categories + services fetched once, passed down
  useEffect(() => {
  ...
  const ActiveIcon = TABS.find(t => t.id === tab)?.icon ?? Package;
    ...
    <div className="p-4 sm:p-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Manage products, services and categories</p>
        </div>
      </div>

      {/* Tab bar */}
      <div className="flex items-center gap-1 p-1 bg-gray-100 dark:bg-gray-800 rounded-2xl w-full sm:w-fit">
        {TABS.map(({ id, label, icon: Icon }) => (
          <button
            key={id}
            onClick={() => setTab(id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 flex-1 sm:flex-none justify-center sm:justify-start ${
              tab === id
